...
A árvore V2 remove o `.env` rastreado, configuração de banco com valores fixos, chave de dispositivo embutida, segredo padrão de sessão, logs de cabeçalhos/tokens e rota `/debug`. O código antigo foi substituído, não exposto em rotas de compatibilidade. O `MQ-FIRE-TG` foi criado com histórico independente, importando apenas os arquivos da versão revisada. Não foram revogadas credenciais nos serviços externos nem alterados os commits do repositório anterior.
...
3. Usar a versão revisada do `MQ-FIRE-TG` e revisar o diff da **árvore atual**. `npm run check:secrets` é uma barreira básica, não um detector completo. Nunca fazer merge ou mirror dos commits legados neste repositório.
4. No repositório anterior, combinar com colaboradores a janela de eventual reescrita de histórico. Usar ferramenta apropriada, mapa de substituições mantido fora do Git e revisão de todas as refs afetadas. Isso altera SHAs e exige coordenação de clones/PRs. Não executar force-push indiscriminado nem apagar branches de trabalho sem acordo.
...
O caderno experimental registra também planejamento, início, anotações e conclusão em auditoria, sem copiar textos livres para o log. Protocolo, observações e conclusão permanecem privados na conta, mas serão incluídos na exportação solicitada pelo proprietário; esses campos podem conter dados identificáveis. Pseudonimizar antes de publicar. A interface não permite apagar/editar anotações, porém quem administra o banco ainda pode fazê-lo: não é log inviolável.
