# MQ-FIRE TG — laboratório acadêmico reproduzível

[![Validação TG](https://github.com/Vini-Gregorio/MQ-FIRE-TG/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/Vini-Gregorio/MQ-FIRE-TG/actions/workflows/ci.yml)
...
**Condução atual: Vinícius Gregório.** Este é o repositório público independente `Vini-Gregorio/MQ-FIRE-TG`, criado em 22/09/2026. Créditos anteriores e escopo da manutenção estão em [AUTHORS.md](AUTHORS.md); a origem da cópia revisada está registrada em [proveniência](docs/REPOSITORY-INDEPENDENCE.md).

> V2 em validação. Esta refatoração altera API e esquema de dados. Use um **banco novo** e atualize o firmware junto com o backend. Não substitua uma instalação existente sem backup, revisão e ensaio. O histórico Git anterior não foi importado; credenciais já expostas no projeto antigo ainda exigem revogação nos provedores. Leia [SECURITY.md](SECURITY.md).
...
```sh
git clone https://github.com/Vini-Gregorio/MQ-FIRE-TG.git
cd MQ-FIRE-TG
...
Projeto original com contribuições de **Amanda do Prado** e **Vinícius Gregório**, detalhadas em [AUTHORS.md](AUTHORS.md). A evolução acadêmica atual é conduzida por Vinícius; o crédito anterior não implica participação ou manutenção atual. A importação parte da versão acadêmica revisada `c9a9bb2` do projeto anterior, sem seus commits antigos. Telas e APIs foram substituídas na refatoração. A definição de licença continua pendente.
