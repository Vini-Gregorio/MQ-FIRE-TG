...
O [caderno de ensaios](EXPERIMENTS.md) implementa o registro de método prévio, dispositivos, montagem, observações independentes e conclusão explícita. Ele organiza os dados para as prioridades abaixo; não substitui sua execução em hardware. O [repositório independente MQ-FIRE-TG](REPOSITORY-INDEPENDENCE.md) foi criado com os arquivos revisados e histórico novo. O fork anterior permanece preservado durante a transição.
...
| Atualização de banco sem verificar dados existentes | Migrações aditivas e ensaio 001 → 002 → 003 na CI | Backup/restore operacional ainda deve ser exercitado |
...
Acompanhamento no GitHub: [#7 — credenciais e reprodução](https://github.com/Vini-Gregorio/MQ-FIRE-TG/issues/7), [#8 — bancada com múltiplos canais/dispositivos](https://github.com/Vini-Gregorio/MQ-FIRE-TG/issues/8) e [#9 — evidências do TG e pesquisa de IA](https://github.com/Vini-Gregorio/MQ-FIRE-TG/issues/9). Cada tarefa tem critérios de aceite; abertura não significa execução concluída.

1. **P0 — encerrar a exposição e reproduzir a instalação.** O responsável revoga os segredos antigos nos provedores; outro colega segue o README deste novo repositório em ambiente limpo. Registrar commit, configuração sem segredos, CI e resultado. Ensaiar restauração de backup antes de qualquer corte. Aceite: nenhuma dependência de credencial legada e instalação repetida a partir do guia. A eventual limpeza coordenada do histórico anterior vem depois da revogação, sem apagar autoria.
